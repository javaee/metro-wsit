...
public class PolicyMapUtil {
    ...
    public static void rejectAlternatives(PolicyMap map) throws PolicyException {
        ...
        for (PolicyMapKey key: keys) {
            Policy policy = map.getServiceEffectivePolicy(key);
        ...
        for (PolicyMapKey key: keys) {
            Policy policy = map.getOperationEffectivePolicy(key);
            if (policy.getNumberOfAssertionSets() > 1) {
                throw new PolicyException(LocalizationMessages.RECONFIGURE_ALTERNATIVES(policy.getIdOrName()));
            }
        }
        ...
        for (PolicyMapKey key: keys) {
            Policy policy = map.getInputMessageEffectivePolicy(key);
            if (policy.getNumberOfAssertionSets() > 1) {
                throw new PolicyException(LocalizationMessages.RECONFIGURE_ALTERNATIVES(policy.getIdOrName()));
            }
        }
        ...
        for (PolicyMapKey key: keys) {
            Policy policy = map.getOutputMessageEffectivePolicy(key);
            if (policy.getNumberOfAssertionSets() > 1) {
                throw new PolicyException(LocalizationMessages.RECONFIGURE_ALTERNATIVES(policy.getIdOrName()));
            }
        }
        ...
        for (PolicyMapKey key: keys) {
            Policy policy = map.getFaultMessageEffectivePolicy(key);
            if (policy.getNumberOfAssertionSets() > 1) {
                throw new PolicyException(LocalizationMessages.RECONFIGURE_ALTERNATIVES(policy.getIdOrName()));
            }
        }
