/*
 * $Id: SignatureProcessor.java,v 1.8.2.1 2006-09-07 16:37:37 kumarjayanti Exp $
 */
        ...
        try{
            ...
            if (PolicyTypeUtil.usernameTokenPolicy(keyBinding)) {
                ...
                if (PolicyTypeUtil.x509CertificateBinding(originalKeyBinding)) {
                 ...
                 signingKey = new SecretKeySpec(context.getTrustContext().getProofKey(), secretKeyAlg); 
                 keyInfo = prepareForSymmetricKeySignature(
                ...
                if (privKBinding == null) {
                    throw new XWSSecurityException("PrivateKey binding not set for SAML Policy " +
                            " by CallbackHandler");
                }
...
//            XMLUtils.circumventBug2650(context.getSecurableSoapMessage().getSOAPPart());
            ...
            if(logger.getLevel() == Level.FINEST){
                logger.log(Level.FINEST,"Error occurred during signature" +
                        "verification "+ex.getMessage());
            }
            throw new XWSSecurityException(ex);
        }finally{
        ...
        if(signedDataList.size() == 0){
            ...
            for(int i=0;i<signedDataList.size();i++){
                ...
                if(!found){
                    ...
                    throw new XWSSecurityException("SingatureTarget in the message "+
        ...
        try {
            if ( PolicyTypeUtil.derivedTokenKeyBinding(keyBinding)) {
                ...
                if (PolicyTypeUtil.x509CertificateBinding(originalKeyBinding)) {
                    ...
                    boolean includeIST = (com.sun.xml.ws.security.policy.Token.INCLUDE_ALWAYS_TO_RECIPIENT.
                    equals(ikb.getIncludeToken()) || com.sun.xml.ws.security.policy.Token.INCLUDE_ALWAYS.
    ...
    public static SOAPElement insertSCT(FilterProcessingContext context, SecureConversationTokenKeyBinding sctBinding, SecurityTokenReference secTokenRef)
        ...
        if(com.sun.xml.ws.security.policy.Token.INCLUDE_ALWAYS_TO_RECIPIENT.equals(sctBinding.getIncludeToken()) ||
                        com.sun.xml.ws.security.policy.Token.INCLUDE_ALWAYS.equals(sctBinding.getIncludeToken())) {
    ...
    private static KeyInfo handleX509Binding(FilterProcessingContext context, 
        ...
        try{
            if(referenceType.equals("Direct")){
                ...
                reference.setValueType(MessageConstants.X509v3_NS);
                ...
                if(token == null){
                    token = new X509SecurityToken(secureMessage.getSOAPPart(),certInfo.getX509Certificate(),id);
